...
#     # Extract repository name and sanitize it for use
#     repo_name=$(basename "$repo_url" .git | tr '[:upper:]' '[:lower:]' | tr -cd '[:alnum:]-')

#     # Set up project directory
#     project_dir="$REPOCATE_WORKSPACE/$repo_name/source_code"
...
ensure_repo() {
    ...
    # Create necessary directories if they don't exist
    mkdir -p "$REPOCATE_WORKSPACE/$repo_name/container_configs" "$project_dir"

    # Clone or update the repository
    if [[ ! -d "$project_dir/.git" ]]; then
        log "INFO" "Cloning repository $repo_url into $project_dir"
        git clone --verbose "$repo_url" "$project_dir" || error_exit "Failed to clone repository"
    else
        log "INFO" "Updating repository $repo_url in $project_dir"
        (cd "$project_dir" && git pull --verbose) || log "WARN" "Failed to update repository"
    fi
...
### issue with too many colons in -v specifically project_dir
# init_container() {
#     ensure_user_in_docker_group
#     local repo_url=$1
#     local repo_name
...
#     # Ensure there are no special characters or excessive colons in the path
#     if [[ "$project_dir" =~ : ]]; then
#         error_exit "The project directory path contains a colon, which is not allowed."
#     fi
...
#         "$BASE_IMAGE" \
#         /bin/sh -c "cp -r /source/. /workspace/" || error_exit "Failed to copy files to Docker volume"
...
init_container() {
    ...
    # Set up project directory and container name
    project_dir=$(ensure_repo "$repo_url")
    project_dir=$(realpath "$project_dir")  # Convert to an absolute path
    container_name="${repo_name}"
    ...
    # Use bind mounts directly
    log "INFO" "Creating new container $container_name with bind mounts"
    container_id=$(docker run -d \
